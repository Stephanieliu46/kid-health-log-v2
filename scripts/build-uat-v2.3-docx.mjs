...
const txtPath = path.join(repoRoot, "Kid Health Log - UAT Test Cases - v2.3.txt");
const outPath = path.join(
  path.dirname(repoRoot),
  "Kid Health Log - UAT Test Cases - v2.3.docx",
);
...
for (const line of lines) {
  ...
  if (!trim) continue;
  if (/^UAT 用户|^更新说明|^验收记录|^汇总/.test(trim)) continue;
...
sectionBlocks.push(
  new Paragraph({
    ...
    children: [new TextRun({ text: "v2.3 Changelog", bold: true, size: 22 })],
  }),
);
