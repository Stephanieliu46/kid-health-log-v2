...
$txtPath = Join-Path $repoRoot 'Kid Health Log - UAT Test Cases - v2.2.txt'
$outPath = Join-Path (Split-Path $repoRoot -Parent) 'Kid Health Log - UAT Test Cases - v2.2.docx'
...
foreach ($line in $lines) {
  $trim = $line.Trim()
  if ($trim -eq '') { continue }
  if ($trim -match '^UAT 用户|^更新说明|^验收记录|^汇总') { continue }

  if ($trim -match '\(Guest Flow|\(Episode CRUD|\(Log CRUD|\(Data Lifecycle|\(Pro Purchase|\(Legal & Medical Disclaimer|\(Dashboard Analytics|\(Temperature Unit') {
...
try {
  ...
  $chgTitle.Range.Text = 'v2.2 Changelog'
